...
export default function PropertySlugPage() {
  ...
  const { data, loading, error } = usePropertyBySlugQuery({
  ...
  const handleDownloadFlyer = async () => {
    if (!data?.propertyBySlug) return;

    try {
      const result = await generateFlyer({
        variables: { propertyId: data.propertyBySlug.id },
      });
  ...
  const { data: similarData } = useSearchPropertiesQuery({
    variables: {
      filters: {
        cityId: data?.propertyBySlug?.cityId,
      },
      page: 1,
      limit: 4,
    },
    skip: !data?.propertyBySlug?.cityId,
  });
  ...
  // Initialize map
  useEffect(() => {
    if (!data?.propertyBySlug || !mapContainer.current || map.current) return;
